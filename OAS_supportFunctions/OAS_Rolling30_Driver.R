OAS_Rolling30_Driver <- function(){
  ...
  DelByPagePosSectionGeo <- DelByPagePosSectionGeo %>% mutate(international = ifelse(Country %in% c('US','CA'), 0,1), date = end_date) %>% mutate(key = paste0("OAS_",business,campaign,Geo,Pos,section,date))
