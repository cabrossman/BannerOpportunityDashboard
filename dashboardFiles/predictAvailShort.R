...
DelByPagePosSectionGeo <- DelByPagePosSectionGeo %>% mutate(imps = EstImpByPosSecGeo*(365/84)*.85,international = ifelse(Country %in% c('US','CA'), 0,1), date = end_date) %>% mutate(key = paste0(campaign,Geo,Pos,section,date))
